...
### Done (2026-06-23)
- **Redo agents:** USDA, Open Philanthropy, CZI all completed on retry (earlier failures were rate/session limits + intermittent content-classifier unavailability forcing blocks; reworded prompts more neutrally for USDA and the government-air thread). All 14 agencies + 10 foundations now written.
- **Air/PPE/UV/glycols theme** (`themes/air-ppe-uv-glycols.md`): two research threads.
  - *Philanthropy side:* the first movers on airborne-mitigation engineering controls were **biosecurity/EA-aligned philanthropy** (Open Philanthropy/Coefficient — far-UVC, next-gen PPE; **Blueprint Biosecurity** — far-UVC/PPE/glycol "AIR" umbrella, incl. the **GlycolISER** ~$4.5M/7-grantee glycol-vapor program, Mar 2026) and **crypto-philanthropy** (**Balvi**/Vitalik Buterin — $9.4M U Maryland germicidal-UV, $15M UCSD Airborne Institute, Corsi-Rosenthal/clean-air). Columbia/David Brenner far-UVC seeded by the **Shostack Foundation** (+NIH), not Gates. Rockefeller funds air *monitoring* not disinfection; Gates essentially absent.
  - *Government side:* **NIOSH/NPPTL** (respirator certification + elastomeric/PAPR R&D), **BARDA/ASPR** ($6.1B SNS supplies; **DPA Title III** ~$3.2B/182 companies incl. 3M $126M+$76M; **Battelle** $415M mask-decon, Apr 2020), **ARPA-H BREATHE** (up to $156M, indoor-air biosensors — Mayo/Poppy Health/SafeTraces/Virginia Tech), **CDC+EPA** Clean Air in Buildings + ESSER (~$5.5B school HVAC/filtration), **EPA** Grignard Pure (triethylene-glycol air treatment, FIFRA §18 emergency exemptions Jan 2021), **NIH** UVGI grants (far-UVC; upper-room UVGI for TB).
- **CSV database** (`data/`): three extraction agents read every MD file and emitted partial CSVs, merged into:
  - `data/programs.csv` — **302** programmes/initiatives (funder, type, program, acronym, theme, pandemic_relevance, years, status, source_file).
  - `data/awards.csv` — **606** grants/contracts/awards (funder, type, program, recipient, amount, amount_basis, year, theme, purpose, source_file).
  - Both validated (0 malformed rows). `data/README.md` documents schema + caveats (amounts not summable; ceiling vs obligated mixed; representative not exhaustive).
- **Status:** Phases 1–4 complete and pushed. Pandemic-relevant airborne-mitigation theme covered; database in place for filtering/analysis.
